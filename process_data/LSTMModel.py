...
class LSTMModel:
    def __init__(self, sess, vocab, batch_size, seq_len,
                 cell_size, num_layers, test=False):
        self.sess = sess
    ...
    def build_graph(self, test):
        ...
        self.targets = tf.placeholder(
            tf.int32, [self.batch_size, self.seq_len])
        ...
        with tf.variable_scope('lstm_vars'):
            ...
            with tf.device('/cpu:0'):
                ...
                inputs_split = [tf.squeeze(input_, [1])
        ...
        lstm_outputs_split, self.final_state =
        seq2seq.rnn_decoder(inputs_split,
                            self.initial_state,
                            self.cell,
                            loop_function=loop if test else None,
                            scope='lstm_vars')
        lstm_outputs = tf.reshape(
            tf.concat(1, lstm_outputs_split), [-1, self.cell_size])

        logits = tf.matmul(lstm_outputs, self.ws) + self.bs
        self.probs = tf.nn.softmax(logits)

        total_loss = seq2seq.sequence_loss_by_example([logits], [tf.reshape(
            self.targets, [-1])], [tf.ones([self.batch_size * self.seq_len])],
            self.vocab_size)
        self.loss = tf.reduce_sum(total_loss) / self.batch_size / self.seq_len

        self.global_step = tf.Variable(0, trainable=False, name='global_step')
        self.optimizer = tf.train.AdamOptimizer(
            learning_rate=c.L_RATE, name='optimizer')
